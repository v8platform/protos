...
func GenerateFile(gen *protogen.Plugin, file *protogen.File) *protogen.GeneratedFile {
	filename := file.GeneratedFilenamePrefix + ".ras.pb.go"
...
func genMessage(gen *protogen.Plugin, g *protogen.GeneratedFile, f *protogen.File, m *protogen.Message) {
...
func genUnmarshal(g *protogen.GeneratedFile, m *protogen.Message) {
	g.P("func (x *", m.GoIdent, ") UnmarshalRas(reader io.Reader, version int32) error {")
...
// }
//
func fieldStarlarkConverterPrepare(g *protogen.GeneratedFile, m *protogen.Message, field *protogen.Field) {

	if field.Message == nil {
		return
	}

	// log.Println("FullName", field.Message.Desc.FullName())
	log.Println(len(field.Message.Fields))
	log.Println(field.Desc.FullName())

	for _, f := range field.Message.Fields {
		println(f.GoName)
	}

	// ext := GetMessageExtensionFor(field.Message.Desc)
	// if ext == nil {
	// 	log.Println("ext", "not found")
	//
	// 	return
	// }
	// log.Println("ext", ext)

	// log.Println("Options count", len(field.Desc.Options().(*descriptorpb.FieldOptions).UninterpretedOption))
	// log.Println("FullName", field.Desc.FullName())

	for _, f := range field.Message.Extensions {
		log.Println(f.GoName)
	}

	if field.Desc.IsList() {
		g.P("var size_", field.GoName, " int32")
		g.P("for i := 0; i < size_", field.GoName, "; i++ {")
		g.P("")
		g.P("}")
		return
	}
...
// }
//
// func GetMessageExtensionFor(messageDesc protoreflect.MessageDescriptor) *fieldExtension {
