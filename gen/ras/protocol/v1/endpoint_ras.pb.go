...
func (x *EndpointClose) GetPacketType() PacketType {
	return PacketType_PACKET_TYPE_ENDPOINT_CLOSE
}

func (x *EndpointClose) Parse(reader io.Reader, version int32) error {
	if x == nil {
		return nil
	}
...
func (x *EndpointClose) Formatter(writer io.Writer, version int32) error {
	if x == nil {
		return nil
	}
	// decode x.EndpointId opts: encoder:"nullable" order:1
...
func NewEndpointMessage(endpoint Endpoint, message EndpointMessageFormatter) (*EndpointMessage, error) {
	buf := &bytes.Buffer{}
...
func (x *EndpointMessage) Unpack(endpoint Endpoint, into EndpointMessageParser) error {
	switch x.GetType() {
